...
shader_init :: proc(
	_vertexShaderFilePath: string,
	_fragmentShaderFilePath: string,
	_geometryShaderFilePath: string = "",
) -> (
	u32,
	bool,
) {
	...
	geometryShaderSource: cstring
	geometryOk: bool

	if _geometryShaderFilePath != "" {
		geometryShaderSource, geometryOk = file_read_to_cstring(_geometryShaderFilePath)
		if !geometryOk {
			return 0, false
		}
	}
	...
	if _geometryShaderFilePath != "" {
		...
		gl.ShaderSource(geometryShader, 1, &geometryShaderSource, nil)
		gl.CompileShader(geometryShader)
		// check for shader compile errors
		gl.GetShaderiv(geometryShader, gl.COMPILE_STATUS, &success)
		if success == 0 {
			gl.GetShaderInfoLog(geometryShader, 512, nil, infoLog)
			fmt.printfln("ERROR::SHADER::GEOMETRY::COMPILATION_FAILED\n%v", infoLog)
			return 0, false
		}
	}
