package main
/*
CHAPTER: 4-3-2 Blending Sorted
TUTORIAL: https://learnopengl.com/Advanced-OpenGL/Blending
SOURCE CODE IN C++: https://learnopengl.com/code_viewer_gh.php?code=src/4.advanced_opengl/3.2.blending_sort/blending_sorted.cpp
*/
...
main::proc(){
	...
	// ⚠️⚠️ DEPTH BUFFER AND BLEND SET IN renderer.odin ⚠️⚠️
    ...
    transparentTexture:u32 = loadTexture("./assets/textures/window.png")
	...
	for !glfw.WindowShouldClose(window){
        ...
        // ---------------------------------------------
		sort_windows_by_distance_from_camera(&windows, &mainCamera)
		...
		for _window in  windows{
			model = glm.mat4(1.0)
			model = glm.mat4Translate(_window)
			shader_set_mat4(shader,"model",&model[0][0])
			gl.DrawArrays(gl.TRIANGLES,0,6)
		}
...
sort_windows_by_distance_from_camera::proc(_windows:^[]glm.vec3, _camera:^Camera){
	for i:int; i < len(_windows) - 1; i += 1{
		for j:int; j < len(_windows) - i - 1; j+= 1{
			if glm.distance(_camera.position,_windows[j]) < glm.distance(_camera.position,_windows[j+1]){
				_pos:glm.vec3 = _windows[j]
