...
main :: proc() {
	...
	shaderSingleColor, shaderSingleColorOk := shader_init(
		"./shaders/stencil_testing.vs",
		"./shaders/stencil_single_color.fs",
	)
	...
	gl.BufferData(
		gl.ARRAY_BUFFER,
		size_of(f32) * len(cubeVertices),
		raw_data(cubeVertices),
		gl.STATIC_DRAW,
	)
	...
	gl.BufferData(
		gl.ARRAY_BUFFER,
		size_of(f32) * len(planeVertices),
		raw_data(planeVertices),
		gl.STATIC_DRAW,
	)
	// cube position attribute
	gl.EnableVertexAttribArray(0)
